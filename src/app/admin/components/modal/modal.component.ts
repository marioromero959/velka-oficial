...
export class ModalComponent implements OnInit {
  ...
  editarProducto() {
    if (this.formularioProducto.invalid || this.tallesProducto.length == 0) {
      ...
      const product = { id: this.id, nombre, categoria, precio, talles: this.tallesProducto, descripcion }
        ...
        (res: any) => {
          if (this.imagenesProducto.length > 0 || this.borradas.length > 0) {
            if (this.imagenesProducto.length > 8) {
              ...
              this.adminSvc.uploadProductImg(this.imagenesProducto, res._id, this.borradas)
                .then((producto) => {
                  this.showSpinner = false
